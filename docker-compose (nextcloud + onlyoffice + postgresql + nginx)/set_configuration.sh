...
if ! grep -q "nginx-server" trusted_domain.tmp; then
    ...
    docker exec -u www-data nextcloud php occ --no-warnings config:system:set trusted_domains $TRUSTED_INDEX --value="nginx-server"
fi
...
docker exec -u www-data nextcloud php occ --no-warnings app:install groupfolders
docker exec -u www-data nextcloud docker-php-ext-install pdo_mysql 
docker exec -u www-data nextcloud docker-php-ext-enable pdo_mysql 
docker exec -u www-data nextcloud php occ --no-warnings app:install sociallogin